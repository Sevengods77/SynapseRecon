...
class GNN_Diffusion(pl.LightningModule):
    ...
    def p_losses(
        ...
        gt_t = target#[:, 4:]
        #gt_r = target[:, :4]
        pred_r = torch.tensor([1, 0, 0 ,0])[None, ...].repeat(gt_t.size()[0], 1).to(self.device)
        gt_r = pred_r.detach().clone()
        ...
        if loss_type == "all":
            ...
            loss = trans_loss * trans_loss_w + transform_pt_cd_loss * transform_pt_cd_loss_w  #+ rot_pt_cd_loss * rot_pt_cd_loss_w \
                #+ transform_pt_cd_loss * transform_pt_cd_loss_w + rot_loss * rot_loss_w + \
    ...
    @torch.no_grad()
    def p_sample_ddpm(self, x, t, t_index, edge_index, pcd_feats, batch):
        ...
        model_mean = sqrt_recip_alphas_t * (
            ...
            * self.forward_with_feats(
                x, t, edge_index, pcd_feats=pcd_feats, batch=batch
            )
    ...
    def p_sample(self, x, t, t_index, edge_index, sampling_func, pcd_feats, batch):
        return sampling_func(x, t, t_index, edge_index, pcd_feats, batch)
